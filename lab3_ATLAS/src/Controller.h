...
public:
    ...
    void set_rank(map<int,int> totalrank){
        for(auto& r : totalrank){
            rank[r.first] = r.second; 
        }
    }
    ...
    bool enqueue(Request& req)
    {
        ...
        //ATALS
        if(rank.find(req.pid) == rank.end()){
            rank.insert(pair<int,int>(req.pid ,0));
        }
        return true;
    }
    ...
    void tick()
    {
        clk++;
        ATALS
        list<Request>:: iterator itr;
        itr = readq.q.begin();
        while(itr != readq.q.end()){
            itr->waiting++;
            //comply to T= 100K
            if(itr->waiting > 100000){
                otherq.q.push_back(*itr);
                itr=readq.q.erase(itr);
            }else{
                itr++;
            }
        }
        itr = writeq.q.begin();
        while(itr != writeq.q.end()){
            itr->waiting++;
            //comply to T= 100K
            if(itr->waiting > 100000){
                otherq.q.push_back(*itr);
                itr=writeq.q.erase(itr);
            }else{
                itr++;
            }
        }
        ...
        auto req = scheduler->get_head(queue->q,rank);
        if ((req == queue->q.end() || !is_ready(req)) && actq.size()==0 ) {
            ...
            //ATALS
            req = scheduler->get_head(queue->q,rank);
        }
        ...
        //ATALS update attained service
        if ( AS.find(req->pid) == AS.end()){
            AS[req->pid] = 1;
        }else{
            AS[req->pid] += 1;
        }
