...
public:
    ...
    list<Request>::iterator get_head(list<Request>& q, map<int,int> para_rank)
    {
        //rank = para_rank;
        ...
        if (type != Type::FRFCFS_PriorHit) {
            ...
            for (auto itr = next(q.begin(), 1); itr != q.end(); itr++)
                head = compare(head, itr,para_rank);
...
private:
    ...
    function<ReqIter(ReqIter, ReqIter, map<int,int>)> compare = [this] (ReqIter req1, ReqIter req2,map<int,int> rank ) {
            ...
            //ATLAS rules based rank first and FRFS;
            //no rank information
            if(rank.find(req1->pid) == rank.end() || rank.find(req2->pid) == rank.end()){
                if (ready1 ^ ready2) {
            ...
            //rank equal
            if (rank[req1->pid] == rank[req2->pid] ){
               if (ready1 ^ ready2) {
                  if (ready1) return req1;
                     return req2;
               }
               ...
               return rank[req1->pid] < rank[req2->pid] ? req1:req2;
            }
            }
            // if (ready1 ^ ready2) {
            //       if (ready1) return req1;
