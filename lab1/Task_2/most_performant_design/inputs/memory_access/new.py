...
def process_tree(x):
    if type(x) == type( [] ):
        ...
        return str(random.randint(0, 0xfff))
    elif x == ', 0($a0)':
        global base,selayer
        base=(base+1)%cap
        #base=random.randint(0,0xfff)
        selayer=(selayer+1)%cap
        #selayer=random.randint(0,0xff)
        return ", {}($a0)".format(hex(offset*(base+selayer^2)))
    else:
        return x
