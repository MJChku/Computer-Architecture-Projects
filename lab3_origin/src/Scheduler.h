...
public:
    ...
    list<Request>::iterator get_head(list<Request>& q,map<int,int> *rank_ptr)
    {
        rank = rank_ptr;
...
private:
    ...
    function<ReqIter(ReqIter, ReqIter)> compare[int(Type::MAX)] = {
        ...
        [this] (ReqIter req1, ReqIter req2) {
            ...
            if(this->rank->find(req1->pid) == this->rank->end() || this->rank->find(req2->pid) == this->rank->end()){
                if (ready1 ^ ready2) {
                  if (ready1) return req1;
                     return req2;
               }
            ...
            if (this->rank->at(req1->pid) == this->rank->at(req2->pid) ){
               if (ready1 ^ ready2) {
                  if (ready1) return req1;
                     return req2;
               }
               ...
               return this->rank->at(req1->pid) < this->rank->at(req2->pid) ? req1:req2;
            }
            }
            // if (ready1 ^ ready2) {
            //     if (ready1) return req1;
