...
Processor::Processor(const Config& configs,
  ...
  cpu_cycles.name("cpu_cycles")
            ...
            .precision(0)
            ;
  instruction_throughput.name("ipc/instruction_throughput")
            .desc("Number of instructions per cpu cycle")
            .precision(5)
            ;
  cpu_cycles = 0;
...
void Processor::get_ipc(){
  for (unsigned int i = 0 ; i < cores.size(); ++i) {
        instruction_throughput += cores[i]->calc_ipc();
      }
}
    ...
    for (unsigned int i = 0 ; i < cores.size(); ++i) {
      if (cores[i]->finished()) {
        for (unsigned int j = 0 ; j < cores.size() ; ++j) {
          instruction_throughput += cores[j]->calc_ipc();
        }
        //printf(instruction_throughput)
        return true;
      }
    }
    return false;
...
{
    //cout << "retired: %ld " << id << " clk" << retired << " ipc " << (double) retired / clk <<endl;
