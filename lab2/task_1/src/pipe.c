...
#include <stdbool.h>
//#define DEBU
int request_read_ins = 0;
uint32_t* read_ins_back = NULL;
int request_read_data = 0;
uint32_t* read_data_back = NULL;
uint32_t temppc;
int request_write_data = 0;
uint32_t*  write_data_back = NULL;
bool ready[3];
uint32_t value[3];
int wait[3] = {-1};
bool END = false;
void set_on_wait(int type, int index){
    wait[type] = index;
}
void fill_pipe(int type, uint32_t returnvalue){
    #ifdef DEBUG
      printf("\nfill pipe %d \n", type);
    #endif
    ready[type] = true;
...
void pipe_cycle()
{
    ...
    if (pipe.branch_recover) {
        ...
        if (pipe.branch_flush >= 4) {
            ...
            ready[1] = false;
            ready[2] = false;
            cancel_on_wait(wait[1]);
            wait[1] = -1;
            cancel_on_wait(wait[2]);
            wait[2] = -1;
        }
...
void pipe_stage_mem()
{
    ...
    uint32_t val = 0;
    if (op->is_mem){
        if(request_read_data == 0){
            l1_read_32(op->mem_addr & ~3, 2);
            request_read_data = 1;
            return;
        }
        if(!ready[2]){
    ...
    switch (op->opcode) {
        ...
        case OP_SB:
            ...
            if(request_write_data == 0){
                l1_write_32(op->mem_addr & ~3, val, 1);
...
void pipe_stage_fetch()
{
    ...
    if(request_read_ins == 0){
        ins_read_32(pipe.PC,0);
        request_read_ins = 1;
        return;
    }
    if(!ready[0]){
    ...
    //op->instruction = read_ins_back;
    //request_ins_back =NULL;
 
    op->instruction = value[0];
    op->pc = pipe.PC;
    pipe.decode_op = op;
