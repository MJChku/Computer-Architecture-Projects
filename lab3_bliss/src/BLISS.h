...
public:
    ...
    void reset_blacklist(){
        blacklist.clear();
        readq.splice(readq.end(),bliss_readq);
        writeq.splice(writeq.end(),bliss_writeq);
        resetCycleLeft = MAX_CYCLE;
    }
    
    void check_interference(int pid ){
        if(counter.find(pid) == counter.end()){
            counter.insert(pair(pid,0));
        }else if (counter.find(pid) < MAX_REQUEST)
        {
            counter[pid]++;
        }else if (counter.find(pid) == MAX_REQUEST){
            blacklist.insert(pair(pid, true));
            blacklist[pid] = true;
        }
    }
    ...
    bool enqueue(Request& req)
    {
        ...
        if(blacklist.find(itr->pid) != blacklist.end()){
           blacklist = true;
        }
        Queue& queue = get_queue(req.type,blacklist);
        ...
        if (!blacklist && req.type == Request::Type::READ && find_if(writeq.q.begin(), writeq.q.end(),
        ...
        if (blacklist && req.type == Request::Type::READ && find_if(bliss_writeq.q.begin(), bliss_writeq.q.end(),
